...
function file_name.get_tail(filename)
    return vim.fn.fnamemodify(filename, ":t")
end

function file_name.split_filename(filename)
    local nodes = {}
    for parent in string.gmatch(filename, "[^/]+/") do
        table.insert(nodes, parent)
    end
    table.insert(nodes, file_name.get_tail(filename))
    return nodes
end

function file_name.reverse_filename(filename)
    local parents = file_name.split_filename(filename)
    reverse(parents)
    return parents
end
...
function file_name.get_unique_filename(filename, other_filenames)
    local rv = ''

    local others_reversed = vim.tbl_map(file_name.reverse_filename, other_filenames)
    local filename_reversed = file_name.reverse_filename(filename)
...
function file_name.get_current_ufn()
    ...
    return file_name.get_unique_filename(current_name, names)
end
...
local comps = {
    ...
    file = {
        info = {
            -- provider = 'file_info',
            provider = file_name.get_current_ufn,
            hl = {
                fg = colors.blue,
                style = 'bold'
            },
